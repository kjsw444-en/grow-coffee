...
$destDir = Join-Path $root '프론트엔드\public\videos'
...
if ($SourceFile -and $DestName) {
  ...
  Write-Output ''
  Write-Output '다음: coffeeVariants.ts / hiddenCoffeeVariants.ts 의 COFFEE_VIDEO_VERSION·HIDDEN_VIDEO_VERSION 을 +1 하고 git add public/videos'
  exit 0
}
...
Copy-ByPattern $hiddenDir '*돌체*커플*' 'coffee-drink-hidden-americano-m-dolce-f.mp4' | Out-Null
...
Write-Output '=== 완료 ==='
Get-ChildItem -LiteralPath $destDir -Filter '*.mp4' | Sort-Object Name | ForEach-Object {
  '{0,-48} {1,7:N2} MB' -f $_.Name, ($_.Length / 1MB)
}

Write-Output ''
Write-Output '다음: git add 프론트엔드/public/videos && 버전 상수 +1 (필요 시)'
